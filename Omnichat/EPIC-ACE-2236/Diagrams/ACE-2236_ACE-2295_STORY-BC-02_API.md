...
| `targetEstimate` | int | เลขเดียวกับที่โชว์ตอน compose (snapshot เก็บโชว์ใน list/detail): โหมด everyone = `reachableCount` (followers − blocks) · โหมด specific = ผล estimate endpoint (decision 2026-07-15) |
...
| โหมด specific + estimate สด > **500 คน** | 200 + `CodeBroadcastValidation` — "audience exceeds the 500-recipient limit (estimated X)" · server นับซ้ำด้วย query เดียวกับ estimate endpoint ณ ตอน save (2026-07-15 — const `MaxAudienceRecipients` · โหมด everyone ไม่จำกัด · คนละเรื่องกับ batch 500/batch ของ BC-03) |
