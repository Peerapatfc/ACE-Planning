...
| Add tag | เลือก tag หลายอัน + สร้าง inline ได้ | Append-only ไม่ลบ tag เดิม (soft-delete) |

> **⚠️ ข้อจำกัด channel (verify กับ repo จริง):** auto-reply **ส่งได้เฉพาะ** channel ที่มี outbound integration: **LINE, Facebook, Instagram, Lazada** — **TikTok** ยังส่งไม่ได้ (`tiktok.strategy.ts` คืน *"not yet supported"*) และ **Shopee** ไม่มี send strategy เลย (มีแค่ stub poller) บน 2 channel นี้ rule จะ **ติด tag ได้** แต่ **ข้าม auto-reply** (log ไว้) ส่วน auto-tag ทำงานได้ทุก channel

### 1.4 Rule Execution Logic

เมื่อ message เข้า ระบบทำตามขั้นตอนนี้ทุกครั้ง:

1. **Sender check:** message เป็นของลูกค้า (`sender_type = 'contact'`) หรือไม่ → evaluate เฉพาะ `'contact'`; ข้าม `'agent'` / `'system'` (background)
2. **Dedup check:** message ซ้ำกับ message เดิมใน 5 วินาทีหรือไม่ → ถ้าใช่ skip (background) — *guard ใหม่ที่ต้องสร้าง; dedup เดิม 24 ชม. อยู่ที่ webhook gateway ไม่ใช่จุดนี้*
...
|------|-----------|------------|
| `sender_type = 'rule'` | message record บอกว่า rule ตอบ ไม่ใช่ agent. **หมายเหตุ:** `sender_type` เป็น plain string (`contact \| agent \| system` ตอนนี้) — `'rule'` เป็นค่าใหม่ระดับ app ไม่ใช่ DB enum; AI reply ตอนนี้ใช้ `'agent'` | KPI agent ผิดพลาด |
| SLA state machine ข้าม rule reply | **ไม่มี** field `first_human_response_at` — FRT คำนวณจาก `sla_first_inbound_at` + SLA state machine + `ConversationSlaEvent.response_time_sec`; message `sender_type='rule'` ต้องไม่ advance state "first agent response" | SLA / FRT metric บวมเกินจริง |
| Non-customer sender skip | evaluate เฉพาะ `sender_type = 'contact'`; ข้าม `agent`/`system` | echo / system message trigger rule ผิดพลาด |
| Message dedup 5s | evaluate ครั้งเดียวต่อ message เดิม (guard ใหม่) | ส่ง auto-reply ซ้ำจาก double-send |
...
#### สถานการณ์: ลูกค้าพิมพ์ "ยกเลิก" มาใน LINE นอกเวลาทำการ

มี rule ตั้งไว้ว่า: keyword contains "ยกเลิก" AND channel = LINE → auto-reply "รับเรื่องแล้ว" + tag "cancellation-risk"
มี rule อีกตัว: business hours = outside → auto-reply "นอกเวลาทำการ" + tag "off-hours"

1. **ลูกค้าส่งข้อความ "ยกเลิก order ครับ" ผ่าน LINE เวลา 22:00:**
   - Sender check ผ่าน (`sender_type = 'contact'`)
   - Dedup check ผ่าน (message ใหม่)
   - Evaluate: Rule A match (keyword + LINE ✓), Rule B match (outside hours ✓)
...
4. **FRT ยังนับต่อ** เพราะ message `sender_type = 'rule'` ไม่ advance state "first agent response" ของ SLA

> *ถ้าเป็น channel ที่ส่งไม่ได้ (TikTok/Shopee) auto-reply ของ Rule A จะถูก **skip** แต่ tag ทั้งสองยังติดให้ตามปกติ*
...
| Active rule limit | สูงสุด 20 active rules ต่อ Workspace (`tenant_id` ใน DB) |
...
| Auto-reply channels | LINE / Facebook / Instagram / Lazada เท่านั้น — TikTok & Shopee ยังส่งไม่ได้ |
| SLA attribution | `sender_type = 'rule'` ต้องไม่ advance state "first agent response" ของ SLA |
| Tag integrity | Append-only (soft-delete), column ใหม่ `tagged_by_type`/`tagged_by_rule_id`, graceful skip ถ้า tag ลบ |

> **Implementation grounding:** ทุก technical claim ใน Epic นี้ verify กับ repo `ace` (branch `dev`) แล้ว จุดสำคัญ: `sender_type`/`status` เป็น plain string ไม่ใช่ DB enum; rule engine ใหม่ hook ที่ `omnichat-service` `messages.service.ts` (post-save); business-hours reuse จาก `packages/shared` + config เก็บ per-tenant ที่ `tenant-service`; การส่ง outbound reuse `StrategyRegistry` per-channel — รายละเอียดระดับไฟล์ดู `story-task-breakdown.md`
...
- **Background guards:** non-customer sender skip (`sender_type ≠ 'contact'`), message dedup 5s window (guard ใหม่)
...
- **Background:** `sender_type = 'rule'`, ไม่ advance state "first agent response" ของ SLA, retry 3 ครั้ง; ข้าม channel ที่ส่งไม่ได้ (TikTok/Shopee)
