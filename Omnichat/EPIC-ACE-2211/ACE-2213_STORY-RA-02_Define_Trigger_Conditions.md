...
- ป้องกัน loop: message จาก bot หรือ system sender ไม่ trigger rule / message ซ้ำกันใน 5 วินาทีถูก deduplicate (background)
...
- **Business hours:** within/outside + day of week + timezone ในช่อง field เดียว
...
- Bot sender skip (background): incoming จาก bot/system ไม่ trigger
- Message deduplication 5s window (background)
...
**9. Bot sender skip (background)**

- Given Rule มี keyword contains 'ยกเลิก'
- When Shopee automated notification ส่งข้อความที่มีคำว่า 'ยกเลิก' เข้า conversation
- Then Rule ไม่ evaluate — log: `skipped: sender_type = bot`
...
- Bot sender ต้องไม่ trigger rule ไม่ว่า keyword จะ match หรือไม่
