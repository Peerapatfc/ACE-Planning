...
- Auto-reply ส่งข้อความผ่าน **channel เดียวกัน** กับที่ลูกค้าส่งมา (LINE reply to LINE, Lazada reply to Lazada) — reuse `StrategyRegistry.pushMessage()` / `ConversationsService.sendMessage()`
- **⚠️ ส่งได้เฉพาะ channel ที่มี outbound integration:** LINE, Facebook, Instagram, Lazada — **TikTok** (`tiktok.strategy.ts` คืน *"not yet supported"*) และ **Shopee** (ไม่มี send strategy) จะถูก **skip + log** ส่วน action อื่น (tag) ยังทำงาน
- รองรับ template variables: `{{customer_name}}`, `{{channel}}`, `{{current_time}}` พร้อม fallback value ต่อ variable เมื่อค่าไม่มี
- **Cooldown** เป็น required field — ไม่อนุญาตให้ save โดยไม่กำหนด และไม่อนุญาตให้ = 0; แสดงเป็นภาษา plain ว่า 'ส่งได้สูงสุด 1 ครั้ง ต่อ X นาที ต่อการสนทนา' (เก็บ state ใน Redis)
...
- สิ่งที่อยู่ใน background (ไม่ expose ใน UI): `sender_type = 'rule'` ใน message record (plain string ไม่ใช่ DB enum), และต้องไม่ advance state "first agent response" ของ SLA (ไม่มี field `first_human_response_at` — FRT มาจาก `sla_first_inbound_at` + SLA state machine)
...
- **Channel-support guard:** skip + log ถ้า channel ส่งไม่ได้ (TikTok/Shopee)
- Retry 3 ครั้ง exponential backoff ถ้า channel API fail (background) — inline `executeWithRetry` (omnichat-gateway) หรือ BullMQ pattern (ai-ingestion)
- `sender_type = 'rule'` ใน message record (background) — handle in ui gracefully (บ่งบอกยังไงก็ได้ว่าเป็น rule ตอบ)
- message `sender_type='rule'` ไม่ advance SLA first-agent-response state (background)
...
Scenario 1
- Given Conversation มาจาก Lazada และ rule trigger
- When auto-reply execute
- Then Message ส่งผ่าน Lazada API ไม่ส่งผ่าน channel อื่น
...
Scenario 3 (channel ส่งไม่ได้)
- Given Conversation มาจาก TikTok หรือ Shopee (ยังไม่มี outbound integration) และ rule trigger
- When auto-reply execute
- Then Skip auto-reply — log: `skipped: channel <type> not supported for sending`; action อื่น (add tag) ยังทำงานปกติ
...
- Then SLA state machine ไม่ advance state "first agent response" (ไม่มี `first_human_response_at` — ใช้ `sla_first_inbound_at` + `ConversationSlaEvent`); FRT นับต่อจนกว่า agent จะ reply จริง

**7. Character limit ต่อ channel**

- Given Admin พิมพ์ข้อความเกิน limit ของ channel เช่น LINE (2,000 chars)
- When พิมพ์ถึง limit
...
- **Preview panel:** resolved real-time ด้วย dummy data (ชื่อ: สมปอง, channel: LINE, เวลา: 20:45)
...
- ส่งผ่าน channel ที่ยังไม่มี outbound integration (TikTok = stub, Shopee = ไม่มี strategy) → log + skip auto-reply, action อื่นยังทำงาน

### Business-Critical "Must Not Break"
- `sender_type` ต้องเป็น `'rule'` เสมอ ห้ามเป็น `'agent'` ไม่ว่ากรณีใด (AI reply ปัจจุบันใช้ `'agent'` — auto-reply ต้องแยก)
- SLA state machine ต้องไม่ถูก advance state "first agent response" โดย auto-reply
...
- Unit: SLA state machine ไม่ advance first-agent-response เมื่อ `sender_type='rule'`
- Unit: channel-support guard (skip + log สำหรับ TikTok/Shopee)
- Unit: character limit validation per channel
- Integration: message delivery API per channel (LINE/FB/IG/Lazada)
