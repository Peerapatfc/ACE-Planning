## Sequence Diagrams — NOTIF-04: Notification Rules Configuration
...
- ครบ 10 แถวเสมอ — 1 แถวต่อ `event_type`
...
sequenceDiagram
  ...
  NotiSvc->>NotiSvc: filter by event_type<br/>row not found → fallback DEFAULT_RULES[event_type]
...
| `sla_breached_team` ที่ `metadata.is_escalation = true` | hardcoded: supervisor เท่านั้น — escalation ข้าม rules.recipients (ดู `NOTIF-04_escalation_v2_sequence.md`) |
| ทุก event อื่น  | ใช้ `recipients` จาก `notification_rules` row + resolve ผ่าน context ตามตารางด้านบน |

---

### 4 — SLA Breached Escalation (v2)

> ย่อจาก [NOTIF-04_escalation_v2_sequence.md](./NOTIF-04_escalation_v2_sequence.md) (single source of truth — rationale, ตารางเทียบ v1, rate-limit semantics, implementation checklist อยู่ที่นั่น)
...
sequenceDiagram
  ...
  OmniSvc->>DB_O: markEscalated() — SELECT conversations breached<br/>longer than escalation_minutes<br/>AND NOT EXISTS conversation_sla_events<br/>(event_type = escalated + same cycle_number)
  ...
  loop per conversation
    ...
    OmniSvc-)NotiSvc: TCP emit { cmd: 'create_notification' }<br/>event_type: sla_breached_team,<br/>metadata: { is_escalation: true, cycle_number, ... }
    NotiSvc->>UserSvc: TCP send { cmd: 'list_workspace_members' }<br/>{ tenant_id, roles: [supervisor] }
    UserSvc-->>NotiSvc: supervisors[]
    loop per supervisor
      NotiSvc->>DB_N: INSERT notifications (always — bell stays complete)
      ...
      alt count <= 5
        ...
        NotiSvc->>NotiSvc: skip WS push only<br/>badge catches up via 30s polling / reconnect fetch<br/>(NOTIF-01 Diagram 9)
      end
    end
  end
```
...
- **Emit 1 ครั้งต่อ conversation** (fire-and-forget) → NotiSvc fan-out เอง — escalation ข้าม `rules.recipients` เหลือ **supervisor เท่านั้น** (`metadata.is_escalation` special-case ใน `checkGlobalRule`, ดูตาราง Diagram 3)
- **ไม่เพิ่ม enum** — ใช้ `event_type: sla_breached_team` เดิม + `metadata.is_escalation` แยก escalation ที่ FE/delivery
- **Rate-limit เฉพาะ WS push** (max 5/นาที/user) — INSERT ครบทุกใบ ไม่มี notification หายเงียบ ไม่ต้องมี retry logic
...
| omnichat-service        | notification-service    | TCP send       | `{ cmd: 'get_escalation_configs' }` (escalation v2 — ครั้งเดียวต่อรอบ cron) |
| omnichat-service        | notification-service    | TCP emit       | `{ cmd: 'create_notification' }` (escalation v2 — 1 emit ต่อ conversation) |
...
| notification-service    | Redis                   | INCR / EXPIRE  | `escalation:push:{user_id}` (max 5/min — limit เฉพาะ WS push)        |
